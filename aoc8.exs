defmodule AOC8 do
  def parse(input) do
    ...
    |> Enum.map(fn arr -> Enum.split(arr, 10) end)
  end

  def count_simple_digits(entries) do
    IO.puts("Counting simple")
    frequencies = entries
    |> Enum.flat_map(fn e -> elem(e, 1) end)
    |> IO.inspect(label: "OUTPUT")
    |> Enum.map(&String.length/1)
    |> IO.inspect(label: "COUNTS")
    |> Enum.frequencies()
    frequencies[2] + frequencies[4] + frequencies[3] + frequencies[7]
  end
  ...
  def decode_output({signals, output}) do
    signals
    |> decode_mapping()
    |> decode_digits(output)
  end

  def decode_mapping(signals) do
    """
    a: 8 -
    b: 6 -
    c: 8 -
    ...
    """
    line_counts = signals
    |> IO.inspect(label: "DECODING NEW SIGNALS")
    |> Enum.map(fn digit -> String.to_charlist(digit) end)
    |> List.flatten()
    |> Enum.frequencies()
    |> IO.inspect(lable: "LINE COUNTS")

    b = elem(Enum.find(line_counts, fn {key, val} -> val == 6 end), 0)
    e = elem(Enum.find(line_counts, fn {key, val} -> val == 4 end), 0)
    f = elem(Enum.find(line_counts, fn {key, val} -> val == 9 end), 0)
    ...
    |> String.to_charlist()
    d = Enum.at(four -- [b, c, f], 0)

    seven = Enum.find(signals, fn s -> String.length(s) == 3 end)
    |> String.to_charlist()
    a = Enum.at(seven -- [c, f], 0)

    g = Enum.at('abcdefg' -- [a,b,c,d,e,f], 0)

    IO.inspect([a,b,c,d,e,f,g], label: "FOUND")

    %{a => ?a, b => ?b, c => ?c, d => ?d, e => ?e, f => ?f, g => ?g}
  end

  def decode_digits(mapping, output) do
    output
    |> Enum.map(fn digit -> decode_digit(mapping, digit) end)
    |> IO.inspect(label: "DIGITS")
  end

  def decode_digit(mapping, digit_string) do
    digit_string
    |> String.to_charlist()
    |> IO.inspect(label: "DECODE DIGIT")
    |> Enum.map(fn digit -> mapping[digit] end)
    |> Enum.sort()
    |> case do
        'abcefg' -> 0
        'cf' -> 1
...
input = "dacefg fegab de dceb bedag dae bcgaefd bdacg fbgcad bgedca | acfebgd de dbagc deagcb
